...
class InspectionDetail extends BaseController
{
    ...
    public function add()
    {
        if ($_POST) {
            ...
            if (!$error) {
                $data = $this->get_insert_data($_POST,['car_id']);

                $data = $this->upload_image_with_data($data, 'banner');
                // dd($data);
                $inspection_detail_id = $this->InspectionDetailModel->insertNew($data);
                $data = [
                    'car_id' => $_POST['car_id'],
    ...
    {
        $where = [
            'inspection_detail.inspection_detail_id' => $inspection_detail_id,
        ];
